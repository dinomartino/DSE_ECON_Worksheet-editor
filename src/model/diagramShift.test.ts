...
import { shiftCurve, shiftedLabel, translateCurvePoints } from './diagramShift';
...
describe('shifting a curve', () => {
  it('copies demand to the right, names it D₁, and marks the new equilibrium', () => {
    ...
    expect(flat(mark.label?.en)).toBe('E_1');
  ...
  it('continues the numbering: D₁ shifts to D₂ and marks E₂', () => {
    ...
    expect(flat(result.diagram.points.at(-1)?.label?.en)).toBe('E_2');
  });

  it('never reuses an equilibrium name: a second shift in one diagram takes the next E', () => {
    const diagram = buildFromTemplate('supply-demand');
    const [demand, supply] = diagram.curves;
    const mint = counter();
    ...
    expect(flat(mark.label?.en)).toBe('E_2');
